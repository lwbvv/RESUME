...
- **대표 업무**
  ...
  - 신기능
    - 퀘스트 모듈 개발
      - 퀘스트 모듈 개발로 유저 리텐션 30% 증가
    - 공동 구매 기능 개발
    - 상품 디테일 페이지 셀렉트 리뷰 섹션 추가
    - 온보딩 페이지 개발
    - 소멸 예정 적립금 페이지 개발
    - 서브 커뮤니티 그룹 프리뷰 섹션 및 리스트 페이지 개발
  - 개편, 고도화 및 리팩토링
    - 상품 프리뷰 컴포넌트 서버드리븐UI로 변경
    ...
    - 앱 아키텍쳐 개선
      ...
      - (2023. 05. 01 기준) 30% 정도의 Feature에 적용
  - Flutter Develop Infra
    - CI/CD 구축 방식 및 프로젝트 세팅 개선
      - 고객사가 늘어날 때 마다 프로젝트 추가해야 되는 상황.
      - 데이터 파편화 제거, CI/CD 구축 자동화로 2~4시간 정도 걸리던 작업을 20분으로 절감
    - 로깅 시스템 개선
      - 기존 Level단위 로그 출력의 불편함을 느껴 자체 로거를 만들고, 로그를 카테고리화 시켜 출력하고 원하는 로그만 볼 수 있게함
    - Safety Decoder, 모델 제너레이터 개발
